...
// API
import { getProgress as getProgressApi } from "../../api/firebase/progress";
...
const HomeDashboardScreen = ({ navigation }) => {
  ...
  const {
    ...
    queryFn: async () => normalizeProgress(await getProgressApi(user?.uid)),
    enabled: !!user?.uid, // auth yoksa sorgulama
    staleTime: 0,
